...
def encode_rgb_to_jpeg_bytes(rgb_array, max_side: int = 1280, quality: int = 85) -> Optional[bytes]:
    """
    將 RGB 影像（numpy array: HxWx3, uint8）縮放至最長邊不超過 max_side，並以 JPEG 編碼回傳 bytes。
    優先用 PIL；若無 PIL 則改用 OpenCV。
    """
    try:
...
def extract_frames_bytes_ffmpeg_pipe(video_path: str, timestamps: List[float], max_side: int = 1280, quality_param: int = 2) -> List[Optional[bytes]]:
    """
    透過 ffmpeg 以管線輸出單張影像為 JPEG（mjpeg 編碼）到 stdout。
    使用 scale 濾鏡嘗試將最長邊限制為 max_side。
    """
    results: List[Optional[bytes]] = [None] * len(timestamps)
    if not has_ffmpeg():
        return results
...
def gather_frames_in_memory(video_path: str, timestamps: List[float], expected_count: int) -> List[Tuple[int, float, bytes]]:
    """
    綜合 OpenCV -> ffmpeg pipe -> MoviePy 三種方法，將影格擷取為記憶體中的 JPEG bytes。
    回傳為 (index(從1開始), timestamp, jpeg_bytes) 的列表，順序依 index。
    """
...
# 主程式（給一般函式呼叫）
# -------------------------------
def analyze_video(api_key: str, video_path: str) -> str:
    print_info("啟動主程式")
    try:
        if not api_key or not isinstance(api_key, str):
            raise ValueError("API Key 不可為空")
        if not ensure_file_exists(video_path):
            print_info("影片路徑不存在或不可讀")
            return "分析失敗：影片路徑無效或不可讀"
